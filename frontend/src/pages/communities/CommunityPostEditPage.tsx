...
export default function CommunityPostEditPage() {
  ...
  const loadPost = async () => {
    ...
    try {
      let comm: CommunityDetail
      const commId = Number(idOrSlug) || 0
      if (commId > 0) {
        comm = await communitiesApi.getById(String(commId))
      } else {
        comm = await communitiesApi.getById(idOrSlug)
      }
      setCommunity(comm)
      const p = await communitiesApi.getPost(comm.id, pid)
      setPost(p)
      setTitle(p.title)
      setBody(p.body || '<p></p>')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!community || !post || !title.trim() || isRichTextEmpty(body)) return
    setError(null)
    setSaving(true)
    try {
      await communitiesApi.updatePost(community.id, post.id, {
        title: title.trim(),
        body: body.trim(),
  ...
  const canEdit = user && post && community && (post.authorId === user.id || community.creatorId === user.id)
  ...
  if (!canEdit) {
    return (
      <div className="max-w-xl mx-auto px-4 py-8 text-center">
        <p className="text-[var(--theme-text-muted)]">{t('common.forbidden')}</p>
        <Link
          to={`/communities/${community.slug || community.id}/posts/${post.id}`}
          className="text-[var(--theme-primary)] hover:underline mt-2 inline-block"
        >
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6">
      ...
      <h1 className="text-xl font-bold text-[var(--theme-text)] mb-6">{t('communities.editPost')}</h1>
