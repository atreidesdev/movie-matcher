...
import { mockStudios, getStudioProjects } from '@/mock/mockData'
import { entitiesApi } from '@/api/entities'
import type { MediaTypeForPath } from '@/utils/mediaPaths'
import type { Studio } from '@/types'
import StudioEditModal from '@/components/StudioEditModal'
...
export default function StudioDetail() {
  ...
  useEffect(() => {
    ...
    if (isMockEnabled()) {
      setStudio(mockStudios.find((s) => s.id === numId) ?? null)
      setSections(getStudioProjects(numId))
      setLoading(false)
      return
    }
    setLoading(true)
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 space-y-8">
        ...
        <>
            ...
            <div>
              ...
              {studio.country && (
                <p className="text-gray-600">
                  {t('media.country')}: {studio.country}
                </p>
              )}
            </div>
            {canEdit && (
          ...
          {sections.length > 0 && (
            <div className="space-y-8">
              <h2 className="text-xl font-semibold">{t('studio.projects')}</h2>
      ...
      <StudioEditModal
        ...
        studio={studio}
        onSaved={(updated) => setStudio((prev) => (prev ? { ...prev, ...updated } : null))}
      />
    </div>
  )
}
