...
export interface StudioEditModalProps {
  ...
  onClose: () => void
  studio: Studio | null
  onSaved?: (updated: Studio) => void
}

export default function StudioEditModal({ open, onClose, studio, onSaved }: StudioEditModalProps) {
  ...
  useEffect(() => {
    ...
    setEditCountry(studio.country ?? '')
    setEditPoster(studio.poster ?? '')
    setEditNameI18n(studio.nameI18n ?? {})
    setEditDescriptionI18n(studio.descriptionI18n ?? {})
  }, [open, studio])

  useLockBodyScroll(open)

  const handleSave = async () => {
    if (!studio?.id || saving || !editName.trim()) return
    setSaving(true)
    try {
              ...
              onChange={async (e) => {
                ...
                try {
                  const { path } = await adminApi.uploadFile(file, 'poster', {
                    baseName: buildUploadBaseName(editName, 'studio', studio?.id, 'poster'),
                  })
                  setEditPoster(path)
                } finally {
